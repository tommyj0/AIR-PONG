...
{
  ReadConfig();
  sfVideoMode mode = {m_config.windowConfig.width, m_config.windowConfig.height,
                      32};
  m_window = sfRenderWindow_create(mode, "AIR PONG", sfDefaultStyle, NULL);
  font = sfFont_createFromFile("./res/Roboto-Regular.ttf");
...
static void UpdateText()
{
  ...
  if (m_currentFrame % 60 == 0) {
    ...
    fps *= 1e3;
    last = curr;
    snprintf(counter, sizeof(counter), "FPS: %.0f", fps);

    sfText_setString(fps_counter, counter);
  }
  snprintf(buf, sizeof(buf), "%d", m_player->cScore->score);
...
static void spawnPlayer()
{
  ...
  m_player->cShape =
      com_CreateRectangle((Vec2){0, 0}, m_config.paddleConfig.width,
                          m_config.paddleConfig.height, m_white);
  m_player->cBoundingBox = com_CreateBounding(
      (Vec2){m_config.paddleConfig.width, m_config.paddleConfig.height});
  ...
  m_player2->cShape =
      com_CreateRectangle((Vec2){0, 0}, m_config.paddleConfig.width,
                          m_config.paddleConfig.height, m_white);
  m_player2->cBoundingBox = com_CreateBounding(
      (Vec2){m_config.paddleConfig.width, m_config.paddleConfig.height});
}
...
static void spawnBall()
{
  ...
  b->cShape =
      com_CreateCircle((Vec2){width / 2, height / 2},
                       m_config.ballConfig.radius, 20, m_white, 0, m_white);
  b->cBoundingBox = com_CreateBounding(
      (Vec2){m_config.ballConfig.radius * 2, m_config.ballConfig.radius * 2});
}
  ...
  for (int i = 0; i < em_GetTotalEntities(); ++i) {
    ...
    if (ent_GetTag(e) != BALL) {
      ...
      if (overlap.x > 0 || overlap.y > 0) {
        ...
        diff = (diff * (PI * 0.9)) /
               (ball->cBoundingBox->size.y + e->cBoundingBox->size.y);
        float angle = ent_GetTag(e) == PLAYER2 ? -diff + PI : diff;
        ball->cTransform->vel =
            Vec2_Polar(Vec2_mag(ball->cTransform->vel) + 0.5,
                       Vec2_angle(ball->cTransform->vel));
